...
- **Админ-трейдер** (`TELEGRAM_ADMIN_IDS`) — публикация и управление **своими** сигналами в **основную ленту**, **добавление/настройка трекера Hash Hedge** через «+» (по умолчанию трекера нет). Переведённый в **кандидаты** (ротация) — **не** попадает в список автоматически: как все, через `POST /cult-candidates/me` (подписка CULT + Bybit). Новости, CULT-каналы, purge — только главный админ. Без `TELEGRAM_SUPER_ADMIN_IDS` все из `TELEGRAM_ADMIN_IDS` — полный доступ (обратная совместимость).
...
**Кто не может вступить:** админы в **ТОП** (`main_feed_publisher`) — публикуют в основную ленту; CTA **«Стать кандидатом»** скрыт (`CultCandidateJoinPanel`). Переведённые в кандидаты ротацией — **тот же** путь вступления через API, запись в `cult_candidates` не создаётся при `PUT /traders/{id}/roster`.

...
def assert_can_disconnect_copy(db: Session, row: UserBybitSettings) -> None:
    """Нельзя сбросить baseline, пока есть неоплаченная комиссия."""
    if has_unpaid_copy_invoice(db, row.telegram_user_id):
        raise ValueError("Сначала оплатите счёт copy-trading")
    profit = unbilled_profit(row.last_equity_usd, row)
    if profit >= MIN_FEE_USD:
        fee = fee_from_profit(profit)
        raise ValueError(f"Сначала оплатите комиссию ${fee:g} с прибыли copy-trading")

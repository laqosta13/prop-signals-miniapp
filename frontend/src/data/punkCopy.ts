...
export const PUNK_VOLNOVOI_SUBTITLE_LEAD = "копирует все операции";
export const PUNK_VOLNOVOI_SUBTITLE_ACCENT = "ТОП-операторов";
...
export const PUNK_DISCLAIMER_POINTS = [
  ...
  "Сеть показывает операции открыто, не заряжать все 100% депо в одну операцию, даже если оператор самый топовый — любая сделка может быть провальной.",
] as const;
...
export const PUNK_HASH_HEDGE_TRADE_HINT = "Скопировать код сделки и открыть Hash Hedge в браузере Telegram";
export const PUNK_HASH_HEDGE_TRADE_COPIED =
  "Полный код в буфере. Уровни на карточке — тап по цифре копирует вход, стоп или цель.";

...
function buildCopy(theme: Theme): ThemedCopy {
  ...
  if (!punk) {
    return {
      ...
      feedSubBanner:
        "Активные сделки доступны по подписке. Оформите доступ во вкладке «Подписка».",
      feedSubBannerBtn: "Оплата и подписка →",
      feedEmpty: "Пока нет сделок.",
      feedEmptyClosed: "Пока нет отработанных сделок.",
